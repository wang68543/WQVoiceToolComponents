...
//    NSArray *paths               = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
//    NSString *documentPath       = [paths objectAtIndex:0];
//    NSString *wavFile        = [documentPath stringByAppendingPathComponent:[NSString stringWithFormat:@"11.amr"]];
//    WQLog(@"documentPath=%@", documentPath);
...
//    [data writeToFile:wavFile atomically:YES];
   ...
   char * buf = [data bytes];
    ...
    //这时取出来的是纯pcm数据
    buf += nPos;
  NSData *amrData = EncodePCMToAMR(buf,(int)(maxLen- nPos),nChannels,nBitsPerSample);
//    return EncodePCMToAMR(buf,(int)(maxLen- nPos),nChannels,nBitsPerSample);
//    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES);
//    NSString *path = [[paths lastObject] stringByAppendingPathComponent:@"recordWavToAmr22.amr"];
